...
<head>
    ...
    <title>Edit Device - Biometric System</title>
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center py-6">
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600">Update device information for {{ $device->device_name ?: $device->serial_number }}</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('biometric.devices.update', $device) }}" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="device_type" id="device_type" required
                                    ...
                                    <option value="zkteco" {{ old('device_type', $device->device_type) == 'zkteco' ? 'selected' : '' }}>ZKTeco</option>
                                    <option value="honeywell" {{ old('device_type', $device->device_type) == 'honeywell' ? 'selected' : '' }}>Honeywell</option>
                                    <option value="morpho" {{ old('device_type', $device->device_type) == 'morpho' ? 'selected' : '' }}>Morpho</option>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="language" id="language"
                                       value="{{ old('language', $device->language) }}"
                                       placeholder="e.g., 69"
                        ...
                        <div>
                            ...
                            <input type="text" name="push_version" id="push_version"
                                   value="{{ old('push_version', $device->push_version) }}"
                        ...
                        <div>
                            ...
                            <textarea name="notes" id="notes" rows="3"
                                      ...
                                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 @error('notes') border-red-300 @enderror">{{ old('notes', $device->notes) }}</textarea>
                            @error('notes')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
